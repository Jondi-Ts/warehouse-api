...
from app.crud import stock_crud, products_crud
...
def create_stock_endpoint(stock: schemas.StockCreate, db: Session = Depends(database.get_db)):
    db_product = products_crud.get_product_by_id(db, stock.product_id)
    if not db_product:
...
def get_stock_endpoint(skip: int = 0, limit: int = 10, db: Session = Depends(database.get_db)):
    return stock_crud.get_stock(db, skip=skip, limit=limit)
...
def get_stock_by_product_endpoint(product_id: int, db: Session = Depends(database.get_db)):
    db_stock = stock_crud.get_stock_by_product_id(db, product_id)
    if db_stock is None:
...
def reduce_stock_endpoint(stock_id: int, quantity: int, db: Session = Depends(database.get_db)):
    db_stock = stock_crud.get_stock_by_id(db, stock_id)
    if not db_stock:
        raise HTTPException(status_code=404, detail="Stock entry not found")
...
def delete_stock_endpoint(stock_id: int, db: Session = Depends(database.get_db)):
    db_stock = stock_crud.delete_stock(db, stock_id)
    if db_stock is None:
        raise HTTPException(status_code=404, detail="Stock entry not found")
    return db_stock
...
def get_low_stock_products(minimum_quantity: int = 10, db: Session = Depends(database.get_db)):
    return stock_crud.get_products_below_threshold(db, minimum_quantity)
